...
def test_openrouter_openai_route_receives_max_effort_despite_litellm(model_name: str) -> None:
    ...
    # "max" rides as a raw body field (see _reasoning_settings); the effort must
    # be present on the wire-shaped extra_args.
...
def test_reasoning_effort_and_timeout_coexist_in_model_settings() -> None:
    ...
    assert dumped["extra_args"] == {"timeout": 30, "extra_body": {"reasoning_effort": "max"}}
