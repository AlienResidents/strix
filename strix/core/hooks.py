...
# gathered new information or produced output and resets the counter. The
# set is deliberately an allowlist of Strix's local bookkeeping tools:
# SDK-hosted sandbox tools never reach the local tool path, so they can only
# be detected by name from the model's function_call output items, and any
# name missing here fails safe (counts as progress, never as a stall).
...
class ReportUsageHooks(RunHooks[dict[str, Any]]):
    ...
    def _check_stall(
        ...
        when the model's response requests any tool outside the bookkeeping
        allowlist. A sub-agent that ignores the escalating warnings is
        force-stopped by raising, which marks only that agent failed; the
        scan continues. The root agent is never fenced -- it legitimately
        waits on children, and killing it kills the scan.
        """
        ...
        content = (
            ...
            "external action. You are repeating yourself. Take a concrete action now -- "
            "probe the target, read code, or write up what you have -- or call "
            f"agent_finish. At {limit} consecutive stall turns this agent is force-stopped "
            "and its in-progress work is discarded. "
